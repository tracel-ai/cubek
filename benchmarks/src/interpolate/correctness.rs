...
//! since interpolate is a unary operation) so the two `HostData`s they return
...
use crate::interpolate::{problem::problem_for, strategy::strategy_for};
...
pub fn kernel_result(
    ...
    strategy_result(client, problem, seed_lhs)
}
...
pub fn reference_result(
    ...
    let device = <TestRuntime as Runtime>::Device::default();
    let client = <TestRuntime as Runtime>::client(&device);
    cpu_reference_result(client, problem, seed_lhs, progress)
}
