...
//! ```
//!
//! `dtype` is `f32` (default) or `f64`.

use cubecl::prelude::*;
use cubecl::std::tensor::TensorHandle;
use cubecl::{Runtime, TestRuntime, future};

fn run<E: Float + CubeElement>(m: usize, n: usize, iters: usize, label: &str) {
    ...
    cubek_linalg::qr::<TestRuntime, E>(&client, &a).unwrap();
...
fn main() {
    ...
    match dtype.as_str() {
        "f64" => run::<f64>(m, n, iters, "f64"),
        "f32" => run::<f32>(m, n, iters, "f32"),
