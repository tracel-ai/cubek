...
//! launch's own outputs with [`Client::check`].
//!
...
use cubecl::{prelude::Client, server::Handle};
...
where
    F: FnOnce(&Client) -> ExecutionOutcome,
{
    debug_assert!(
        !outputs.is_empty(),
...
impl Unrun {
    ...
    fn of(client: &Client, outputs: &[&Handle]) -> Option<Self> {
