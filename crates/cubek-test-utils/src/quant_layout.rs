...
/// Per-axis block extents, the number of elements each scale covers along each axis. A level with
/// no blocks has one scale for the whole tensor, so its block spans everything.
...
/// two-level scheme grids exactly like a one-level block scheme; its global scale is a separate
...
pub fn values_dtype(scheme: &QuantScheme) -> StorageType {
    match scheme.store {
        QuantStore::PackedU32(_) => StorageType::Scalar(ElemType::UInt(UIntKind::U32)),
...
pub fn scales_dtype(scheme: &QuantScheme) -> StorageType {
    StorageType::Scalar(ElemType::from_quant_param(scheme.param))
}
