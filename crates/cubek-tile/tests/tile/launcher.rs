...
fn arg_right_aligns_batches_and_drops_size_one() {
    ...
    let one_batch = launch
        ...
        .batches(&[B0, B1])
        .build();
    assert!(one_batch.spec.space.contains(B1));
    assert!(!one_batch.spec.space.contains(B0));
    ...
    let broadcast = launch
        ...
        .batches(&[B0, B1])
        .build();
    assert!(!broadcast.spec.space.contains(B0));
    assert!(!broadcast.spec.space.contains(B1));
}
