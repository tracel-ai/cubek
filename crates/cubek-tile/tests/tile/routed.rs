...
//! everywhere else. The leaf sees one value of the axis, so it contracts nothing, and the block
//! it opens folds along `K` exactly as an unrouted one does.
...
/// The contraction the folded leaf walks, wider than one line so the walk has steps.
const DEPTH: usize = 8;
/// The line both operands are served at along `K`; the folded kernel's `Const<4>` is the same
/// number.
const LINE: usize = 4;
...
fn lined_x_values() -> Vec<f32> {
    (0..TOKENS * DEPTH).map(|i| (i % 5) as f32).collect()
}

/// `w[e][n][k]`: each expert's slab transposed against `w_values`, so the weights line along the
/// contraction rather than along the output.
fn lined_w_values() -> Vec<f32> {
    (0..EXPERTS * FEATURES * DEPTH)
        .map(|i| {
            let e = i / (FEATURES * DEPTH);
            let rest = i % (FEATURES * DEPTH);
            ((e + 1) * (rest % 3 + 1)) as f32
        })
        .collect()
}

fn lined_expected(routes: &[u32]) -> Vec<Vec<f32>> {
    let (x, w) = (lined_x_values(), lined_w_values());
    (0..TOKENS)
        .map(|m| {
            let e = routes[m] as usize;
            (0..FEATURES)
                .map(|n| {
                    (0..DEPTH)
                        .map(|k| x[m * DEPTH + k] * w[(e * FEATURES + n) * DEPTH + k])
                        .sum()
                })
                .collect()
        })
...
#[cube(launch)]
fn moe_promoted_kernel<E: Numeric>(
...
fn run_promoted(routes: &[u32]) -> HostData {
    let client = cubecl::test_device().client();
    let f32_ty = f32::elem_type_native();
    let u32_ty = u32::elem_type_native();
    ...
    let (x_handle, _) = TestInput::builder(client.clone(), Shape::new([TOKENS, DEPTH]))
        .dtype(f32_ty)
        .custom(lined_x_values())
        .generate_with_f32_host_data();
    let (w_handle, _) = TestInput::builder(client.clone(), Shape::new([EXPERTS, FEATURES, DEPTH]))
        .dtype(f32_ty)
        .custom(lined_w_values())
        .generate_with_f32_host_data();
    let routes_handle = TestInput::builder(client.clone(), Shape::new([TOKENS]))
        .dtype(u32_ty)
        .custom(routes.iter().map(|&r| r as f32).collect())
...
/// contracted axis it becomes the fastest one, which is then not the axis the operands line
/// along, and the fold every `K`-stored weight needs is off the table: a route can only run one
/// scalar cell at a time, which no vectorization reaches. A refusal inside a cube verb dies on
/// the expansion worker, so a regression here reads as the output keeping what it was filled
/// with rather than as a message.
#[test]
fn a_routed_walk_folds_its_contraction_into_a_register_block() {
    let got = run_promoted(&ROUTES);
    let want = lined_expected(&ROUTES);
    for (m, row) in want.iter().enumerate() {
        for (n, cell) in row.iter().enumerate() {
            assert_eq!(
                got.get_f32(&[m, n]),
