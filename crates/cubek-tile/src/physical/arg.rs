...
/// storage tile that is no level's tile is still read, by the layout walk alone: the projection
/// maps every logical coordinate onto its physical fragments whatever the levels cut. A comptime
/// fact in the kernel; [`at`](crate::Tile::at) makes [`Tiled`](Storage::Tiled)
/// [`Contiguous`](Storage::Contiguous) at the storage tile's level, where it has one.
