//! Where an operand's cells sit at each level of its space ([`Residence`]), the plan that
//! hands one out per level ([`StagePlan`]), and how a materialized level lays its buffer
//! out ([`StageStorage`]). The operand's own half of a [`Tiling::over`](crate::Tiling::over)
//! build: a level states how deeply it buffers, an operand states where it lives.

use cubecl::zspace::SmallVec;

use crate::{Instruction, MAX_LEVELS};
