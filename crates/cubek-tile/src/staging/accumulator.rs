...
//! acc.mm(&a, &b);                   // the contraction, and the drain that closes the scope
//! ```
//!
...
//!
//! ```ignore
//! acc.zero();                       // or acc.seed(op) for a fold's identity
//! acc.mma(&a, &b);
//! ```
//!
//! An output stating nothing is [`InPlace`](Residence::InPlace) and contracts where it already
//! lies. Both read as those two lines: what differs is the operand's statement, not the kernel.
...
impl<EA: Numeric, Out: Numeric> AccumulatorScope<EA, Out> {
    ...
    pub fn seed(&mut self, #[comptime] fold: LeafOp) {
        match self {
            AccumulatorScope::Register { tile, sink: _ } => tile.init_identity(fold),
            AccumulatorScope::InPlace { sink } => sink.init_identity(fold),
        }
    }

    /// `c = a · b`: contract into the accumulator and drain, the contraction owning the init.
    ...
    pub fn mm<Lhs: Numeric, Rhs: Numeric>(&mut self, lhs: &Tile<Lhs>, rhs: &Tile<Rhs>) {
        match self {
            ...
            AccumulatorScope::InPlace { sink } => sink.mm(lhs, rhs),
        }
    }

    /// `c += a · b`: [`mm`](AccumulatorScope::mm) with the accumulate its name carries, folding
    /// onto an accumulator the caller initialized. Drains the same way: the contraction exhausts
    /// the scope the accumulator was opened for, so leaving it is what writes the result back.
    ...
    pub fn reduce_axis_accumulate<In: Numeric>(
        ...
        match self {
            AccumulatorScope::Register { tile, sink } => {
                tile.reduce_axis_accumulate(input, fold);
                tile.drain_cast_into(sink);
            }
            AccumulatorScope::InPlace { sink } => sink.reduce_axis_accumulate(input, fold),
        }
    }
}
...
impl<Acc: Numeric> Tile<Acc> {
    ...
    /// built. The op that closes the scope drains it; where that op is
    /// [`mm`](AccumulatorScope::mm) or [`reduce_axis`](AccumulatorScope::reduce_axis) it owns the
    /// init too, and only the accumulating verbs need the caller to state one
    /// ([`zero`](AccumulatorScope::zero), or [`seed`](AccumulatorScope::seed) for a fold's identity).
    ///
