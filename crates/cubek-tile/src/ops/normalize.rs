...
pub enum TapMask {
    /// Sum only taps whose projected input sample is in bounds, removing edge darkening.
    #[default]
    Masked,
...
impl<T: Float> Tile<T> {
    ...
    /// either through a post-pass would conceal an extra contraction or memory walk.
    pub fn normalized(mut self, #[comptime] mask: TapMask, #[comptime] guard: DivGuard) -> Tile<T> {
        match &mut self.tile_kind {
            TileKind::Procedural(data) => {
                comptime!(assert!(
                    data.factors().is_some(),
                    "Tile::normalized: the procedural recipe states no separable factorization"
                ));
                data.normalization = comptime!(Some((mask, guard)));
            }
            TileKind::Gmem(_)
            | TileKind::Smem(_)
    ...
    pub fn normalize<Meta: Numeric>(
        ...
        match comptime!(acc_space.partitioner().clone()) {
            ...
            Partitioner::Level(_) => {
                let op_space = witnessed_space(acc_space, self, denominator, denominator);
