...
//!
//! The round trip over every value type and shape lives in `round_trip.rs`. What is here is what
//! only a global scale can get wrong: a missing or unexpected binding, a scale read at the
//! compute type instead of at its own, an effective scale that no longer fits the output type,
//! and block scales narrow enough to be worth a second level in the first place.
...
/// The level under test. `store` and `param` are left to the caller: they are what decides whether
/// the block scales are narrow enough to need a global scale at all.
fn two_level(store: QuantStore, param: QuantParam) -> QuantScheme {
    QuantScheme::default()
...
fn packed_f32() -> QuantScheme {
    two_level(QuantStore::PackedU32(0), QuantParam::F32)
}
...
fn fixture(scheme: &QuantScheme) -> Fixture {
    // Block scales spanning what a narrow param holds, under a global scale small enough that
...
fn quantize_rejects_an_unexpected_per_tensor_scale() {
    let scheme = packed_f32().with_level(QuantLevel::block([BLOCK as u8]));
    let f = fixture(&scheme);
...
/// ue4m3 block scales under an f32 global, the configuration the level exists for: one byte per
/// block instead of four, with the global carrying the range that no longer fits.
///
/// Needs native `i8` and e4m3, so it self-skips where either is missing.
#[test]
fn ue4m3_block_scales_with_an_f32_global_round_trip() {
    // Block scales e4m3 represents exactly, so the only error left is the value quantization. A
    ...
    let scheme = two_level(QuantStore::Native, QuantParam::UE4M3);
    let num_blocks = M * N / BLOCK;
