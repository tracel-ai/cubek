...
impl<P: ReducePrecision> ReduceInstruction<P> for Max {
    ...
    fn reduce(
        ...
        match accumulator {
            Accumulator::Packed(packed) => {
                PackedExtremum::descending().reduce::<P>(packed, item, reduce_step)
            }
    ...
    fn plane_reduce_inplace(_this: &Self, accumulator: &mut Accumulator<P>) {
        match accumulator {
            Accumulator::Packed(packed) => {
                PackedExtremum::descending().plane_reduce_inplace::<P>(packed)
            }
            Accumulator::Unpacked { elements, args } => {
    ...
    fn fuse_accumulators(_this: &Self, accumulator: &mut Accumulator<P>, other: &Accumulator<P>) {
        match (accumulator, other) {
            (Accumulator::Packed(packed), Accumulator::Packed(other_packed)) => {
                PackedExtremum::descending().fuse_accumulators::<P>(packed, other_packed.item())
            }
            (
                Accumulator::Unpacked { elements, args },
    ...
    fn to_output_parallel<Out: Numeric, Idx: Numeric>(
        ...
        match accumulator {
            Accumulator::Packed(packed) => {
                PackedExtremum::descending().to_output_parallel::<P, Out, Idx>(packed.item())
            }
    ...
    fn to_output_perpendicular<Out: Numeric, Idx: Numeric>(
        ...
        match accumulator {
            Accumulator::Packed(packed) => {
                PackedExtremum::descending().to_output_perpendicular::<P, Out, Idx>(packed.item())
            }
            Accumulator::Unpacked { elements, args } => {
