...
fn innermost_any_of_accepts_row_and_col_major() {
    ...
    let req =
        LayoutRequest::new().with(Constraint::required(Facet::Innermost(AxisSet::new(&[M, K]))));
...
fn tiled_is_satisfied_by_a_finer_multiple() {
    ...
    let tiled_8 = ConcreteLayout::new(&[PhysicalAxis::tiled(M, 64, 8), PhysicalAxis::untiled(K, 64)]);
    let tiled_16 =
    ...
    let untiled = ConcreteLayout::new(&[PhysicalAxis::untiled(M, 64), PhysicalAxis::untiled(K, 64)]);
