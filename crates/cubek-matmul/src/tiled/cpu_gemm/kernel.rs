...
/// The whole body is the accumulator's scope, the same three lines the tensor-core kernel runs.
/// `out` states no residence, so its scope is [`InPlace`](cubek_tile::Residence::InPlace): the
/// register leaf contracts through `c` itself and there is nothing to drain.
///
/// Each operand arrives as one argument bundling its tensor with its comptime spec; the one
/// kernel [`Space`] is projected onto each in the first lines. `a` stays scalar (broadcast per `K`); `b` and `c` carry the
