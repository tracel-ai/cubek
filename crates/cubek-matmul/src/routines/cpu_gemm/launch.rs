...
use cubek_tile::{
    Axis, ConcreteLayout, CubeAxis, Cut, PhysicalAxis, Schedule, Space, TileArgLaunch, Tiling,
    WalkOrder,
};
...
impl<'a, R: Runtime> Operand<'a, R> {
    ...
    /// Project the operand into a launch and load it via [`TileArgLaunch::from_concrete`]. Builds
    /// its [`ConcreteLayout`] (the *physical* layout: surviving batches single-fragment, `matrix`
    /// expanded per storage-tiling level) for the tile [`Storage`], and the *logical* `axes`
    /// (`[batches…, row, col]`) for the projection — col-major's physical order differs, so the two
    /// aren't interchangeable. Broadcasting is omission: a batch dim kept at size 1 is dropped (axis
    /// and buffer dim both), survivors riding `batch_axis(pad + j)` at their padded output position.
    fn tile_arg<E: Numeric, V: Size>(
        ...
        let (_, rows, cols) = layout.logical_dims(&binding.shape);
        ...
        for (j, &b) in batches.iter().enumerate() {
            if b > 1 {
                ...
                phys.push(PhysicalAxis::new(batch_axis(pad + j), b));
        ...
        TileArgLaunch::from_concrete(binding, &ConcreteLayout::new(&phys), &axes, space, v, check)
    }
}
