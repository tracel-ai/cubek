...
use cubecl::prelude::*;
use cubek_tile::{DeliveryFamily, Tile, TileSpec};
...
/// operand is a plain tensor (or tensor map) plus its comptime [`TileSpec`]; the first
/// lines are the tensor-to-tile transformation. Each operand keeps its own element type,
