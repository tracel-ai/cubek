...
fn decode_gemv_matches_the_reference(field: QuantValue, block: usize) {
    ...
    let (d_out, d_in, rows) = (256, block * 8, 1);
    ...
    for m in 0..d_out {
        ...
        assert!(
            (got[m] - want).abs() <= tolerance,
            "at row {m}: got {}, want {want} (plane {plane})",
            got[m]
        );
    }
}
...
fn eight_bit_weights_decode_against_f16_scales() {
    decode_gemv_matches_the_reference(QuantValue::Q8S, 32);
}

#[test]
fn four_bit_weights_decode_against_f16_scales() {
    decode_gemv_matches_the_reference(QuantValue::Q4S, 32);
}
