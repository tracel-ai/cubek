...
#[cube]
impl Layout for WeightDgradLayout {
    ...
    fn to_source_pos(&self, coords: Self::Coordinates) -> NhwcCoords {
        ...
        let (mut rem, out_c) = self.padded_channels.div_mod(k);
...
impl<'a, R: Runtime> WeightDgradLayoutLaunch<'a, R> {
    pub fn from_args(
        ...
        WeightDgradLayoutLaunch::new(padded_channels, shape_k, shape_n, params, config)
    }
}
