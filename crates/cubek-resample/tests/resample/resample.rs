...
use cubek_resample::definition::{
    BoundaryMode, Kernel, NormalizationMode, Placement, PlacementArg, Resample, ResampleArgs,
    ResampleAxis, Semiring,
};
...
fn resample_1d_identity_test() {
    ...
    let resample_args = ResampleArgs::new().with_placement_arg(PlacementArg::identity());
...
fn resample_1d_test() {
    ...
    let resample_args = ResampleArgs::new().with_placement_arg(PlacementArg::continuous(0.5, 0.0));
...
fn resample_2d_test() {
    ...
    let resample_args = ResampleArgs::new()
        .with_placement_arg(PlacementArg::continuous(0.5, 0.0))
        .with_placement_arg(PlacementArg::continuous(0.5, 0.0));
